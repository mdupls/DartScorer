...
class LeaderView: UIView {
    ...
    private func drawBackground(rect: CGRect, angle: CGFloat, sweep: CGFloat, height: CGFloat) {
        guard let layout = layout else { return }
        ...
        path.addArc(center: center, radiusStart: layout.radius, radiusEnd: layout.radius + height, angle: angle - (sweep / 2), sweep: sweep)
        
        context.addPath(path)
        context.fillPath()
    }
    
    private func drawRound(rect: CGRect, radius: CGFloat, angle: CGFloat, height: CGFloat, name: String) {
        guard let context = UIGraphicsGetCurrentContext() else { return }
        ...
        centreArcPerpendicular(text: name, context: context, radius: radius, angle: -angle, colour: UIColor.white, font: font, clockwise: false)
